...
static void realhrr_ramdbf_finalizer(SEXP ptr)
{
...
SEXP realhrr_ramdbf_create(SEXP n, SEXP m, SEXP tag, SEXP prot) {
    float *p = 0;
    SEXP ptr;
    p = Calloc(INTEGER(n)[0] * INTEGER(m)[0], float);
    ptr = R_MakeExternalPtr(p, install("ramdbptr"), R_NilValue);
    PROTECT(ptr);
    R_RegisterCFinalizerEx(ptr, realhrr_ramdbf_finalizer, TRUE);
    UNPROTECT(1);
    return ptr;
}

SEXP realhrr_ramdbf_get(SEXP ptr, SEXP veclen, SEXP memsize, SEXP vecidx) {
    SEXP ans;
    float *mem = (float*) R_ExternalPtrAddr(ptr);
    double *x;
...
SEXP realhrr_ramdbf_set(SEXP ptr, SEXP veclen, SEXP memsize, SEXP vecidx, SEXP vec) {
    float *mem = (float*) R_ExternalPtrAddr(ptr);
    double *x, s2 = 0;
    int n = INTEGER(veclen)[0];
    int m = INTEGER(memsize)[0];
 ...
 */
SEXP realhrr_ramdbf_dot(SEXP ptr, SEXP veclen, SEXP memsize, SEXP active, SEXP vec) {
    float *mem = (float*) R_ExternalPtrAddr(ptr);
    float *x, *x1, *y;
    double *x0, *a;
    int n = INTEGER(veclen)[0];
 ...
 */
SEXP realhrr_ramdbf_set_rand(SEXP ptr, SEXP veclen, SEXP memsize, SEXP active, SEXP cnormp, SEXP scalep) {

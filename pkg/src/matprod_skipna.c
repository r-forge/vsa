...
/* modified from Radford's version to be callable via .C() (use pointers for int args) */
	                    ...
	                    double *y, int *nry, int *ncy, double *z)
{
    ...
    if (*nrx > 0 && *ncx > 0 && *nry > 0 && *ncy > 0) {
        if (*ncx==1 && *ncy==1)
            z[0] = F77_CALL(ddot)(nrx, x, &int_1, y, &int_1);
        else if (*ncy==1)
            F77_CALL(dgemv)(transT1, nrx, ncx, &one, x, nrx, y, &int_1,
                            &zero, z, &int_1);
        else if (*ncx==1)
            F77_CALL(dgemv)(transT1, nry, ncy, &one, y, nry, x, &int_1,
                            &zero, z, &int_1);
        else
            F77_CALL(dgemm)(transT1, transN2, ncx, ncy, nrx, &one,
                            x, ncx, y, nry, &zero, z, ncx);
    } else /* zero-extent operations should return zeroes */
        for(i = 0; i < (*nrx)*(*ncy); i++) z[i] = 0;
}
